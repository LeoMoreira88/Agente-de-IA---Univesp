    ...
    def retrieve_documents(state: AgentState):
        try:
            ...
            if state['vetorstore'] is None:
                raise ValueError("Nenhum documento foi carregado para consulta")
            ...
            if not docs:
                st.warning("⚠️ Nenhum documento relevante encontrado para a consulta")
    ...
    def generate_answer(state: AgentState):
        ...
        try:
            if state.get('documents'):
                ...
                planilha_present = any(d.metadata.get('tipo') == 'planilha' for d in state.get('documents', []))
                instrucoes_planilha = (
        ...
        if pergunta:
                    ...
                    if 'documents' in user_session and user_session['documents']:
                        with st.expander("📚 Fontes de Referência"):
                            for i, doc in enumerate(user_session['documents'][-5:]):
                                nome = normalizar_nome_arquivo(doc.metadata['source'])
            ...
            if re.search(r'relat[oó]rio', pergunta, re.IGNORECASE) and re.search(r'planilha', pergunta, re.IGNORECASE):
                if user_session.get('uploaded_dataframes'):
                    st.info("Gerando relatório profissional da(s) planilha(s) carregada(s)...")
